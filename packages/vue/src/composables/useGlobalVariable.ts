...
import { inject, isRef, Ref, ref, watch, type MaybeRefOrGetter } from 'vue';
...
export function useGlobalVariable<T extends StreamerbotVariableValue = StreamerbotVariableValue>(
  name: string,
  ...
  if (!isRef(client) || !isRef(status))
    throw new Error(
      'StreamerbotClient not provided. Did you include useStreamerbot in app.vue or another parent component?',
    );
