...
import {
	StreamerbotRequest,
	StreamerbotRequestName,
} from "./types/streamerbot-request.types";
import {
			...
			new Promise<void>(async (res, rej) => {
				...
				try {
					...
					try {
						...
						this.logger?.warn(
							"Error invoking onError handler",
							e,
						);
					}
					rej(error);
	...
	return await withTimeout(
		new Promise<void>((res, rej) => {
			this.socket?.addEventListener(
				...
				() => {
					this.logger?.debug(
						"Disconnected from Streamer.bot WebSocket server",
					);
					res();
				},
				{ signal },
...
private async handshake(): Promise<void> {
	...
	if (!response || !("info" in response))
		throw new Error("Handshake failed (invalid payload)");
	...
	if (
		...
		this.logger?.debug(
			"Connected to Streamer.bot WebSocket server",
			response.info,
		);
...
private async authenticate(data: StreamerbotHelloRequest): Promise<void> {
	...
	this.logger?.debug(
		"Authenticating with Streamer.bot WebSocket server...",
	);
...
protected async onOpen(): Promise<void> {
	...
	try {
		...
		if (!this._authEnabled) {
			void this.getInfo().catch(() =>
				this.logger?.debug(
				"Failed to fetch Streamer.bot instance info",
			),
		);
	}
	...
	if (this.version && this.info) {
		this.logger?.debug(
			`Connected to Streamer.bot: v${this.version} (${this.info.name})`,
		);
	...
	this.options?.onError?.(
		...
		? err
		: new Error("Failed handshake with Streamer.bot"),
	);
...
try {
	...
	if (
		...
		(typeof this.options.subscribe === "object" &&
			!Array.isArray(this.options.subscribe) &&
			Object.keys(this.options.subscribe ?? {}).length)
		) {
			this.logger?.debug(
				"Subscribing to initial events from options:",
				this.options.subscribe,
			);
			await this.subscribe(this.options.subscribe);
		} else if (
			typeof this.options.subscribe === "string" ||
			Array.isArray(this.options.subscribe)
		) {
			this.logger?.debug(
				"Subscribing to initial events from options:",
			...
			const subscriptions =
			await this.getSubscriptionsFromEventStrings(
				this.options.subscribe,
			);
			this.logger?.debug(
				"Parsed subscriptions from options:",
				subscriptions,
			);
			if (subscriptions) {
		...
		if (this.listeners.length) {
			const subscriptions =
			await this.getSubscriptionsFromListeners();
			await this.subscribe(subscriptions);
		}
		
		this.logger?.verbose(
			"Subscribed to requested events",
			this.subscriptions,
			this.listeners,
		);
	} catch (e) {
...
protected onClose(event: CloseEvent): void {
	...
	try {
		if (
			(event.type === "error" || !event.wasClean) &&
			this.options.onError
		)
		...
		this.logger?.warn(
			"Error invoking user-provided onDisconnect handler",
			e,
		);
	}
	...
	if (
		...
		this._reconnectTimeout = setTimeout(
			async () => {
				...
				return;
				this.logger?.debug(
					`Reconnecting... (attempt ${this._retried})`,
				);
				try {
					...
					if (this._retried)
						this.logger?.warn(
						`Failed to reconnect (attempt ${this._retried - 1})`,
						e,
					);
				}
			},
...
protected async onMessage(event: MessageEvent): Promise<void> {
	...
	try {
		...
		this.logger?.warn(
			"Error occurred within user-provided onData callback",
			e,
		);
	}
	...
	if (payload?.event?.source && payload?.event?.type) {
		for (const listener of this.listeners) {
			...
			try {
				...
				this.logger?.warn(
					`Error occurred within user-provided event callback (${listener.events})`,
					e,
				);
			}
		}
...
protected onError(event: Event): void {
	...
	try {
		...
		this.logger?.warn(
			"Error occurred within user-provided onError callback",
			e,
		);
	}
}
...
public async on<TEvent>(
	event: TEvent extends StreamerbotEventName
	? TEvent
	: StreamerbotEventName | "*",
	listener: (
		data: StreamerbotEventPayload<
		TEvent extends StreamerbotEventName
		? TEvent
		: StreamerbotEventName
		>,
	) => void,
...
private async updateSupportedEvents(): Promise<void> {
	...
	try {
		...
		eventsResponse.events as StreamerbotEventsTypeWriteable;
		this.logger?.debug(
			`Successfully fetched supported event types for Streamer.bot v${this.version}`,
		);
	} catch (error) {
		this.logger?.warn(
			"Failed to fetch supported events from Streamer.bot, falling back to stored events type.",
			error,
		);
		this.supportedEvents =
		StreamerbotEvents as StreamerbotEventsTypeWriteable;
	}
}
...
private async getSupportedEvents(): Promise<StreamerbotEventsTypeWriteable> {
	if (!this.supportedEvents) {
		this.logger?.warn(
			"Supported event types not yet initialized, fetching from Streamer.bot instance...",
		);
		await this.updateSupportedEvents();
	}
	return (
...
private getEventsFromListeners(listeners?: typeof this.listeners) {
	return (listeners ?? this.listeners).reduce(
		(acc, listener) => {
...
*/
private async getSubscriptionsFromListeners(
	listeners?: typeof this.listeners,
): Promise<StreamerbotEventsSubscription> {
...
*/
private async getSubscriptionsFromEventStrings(
...
*/
private async parseEventString(
	event: string,
): Promise<
{ source: StreamerbotEventSource; eventTypes: string[] }[] | undefined
> {
	const supportedEvents = await this.getSupportedEvents();
	if (!event || typeof event !== "string") {
		this.logger?.warn(
			`Invalid event subscription requested "${event}"`,
		);
		return;
	}
...
public async subscribe(
	...
	for (const key in events) {
		...
		if (!(key in supportedEvents)) {
			this.logger?.warn(
				`Attempted to subscribe to empty or unknown event source: "${key}"`,
				Object.keys(events),
			);
			continue;
		}
	...
	if (Object.keys(this.subscriptions).length === 0) {
		this.logger?.warn(
			"No valid events to subscribe to. Please provide valid event sources and types.",
		);
		return {
			id: "invalid",
			status: "error",
			error: "No valid events to subscribe to",
		};
	}
...
public async unsubscribe(
			...
			for (const eventType of eventTypes) {
				if (eventType) {
					if (this.subscriptions[eventSource]?.filter) {
						(this.subscriptions[eventSource] = this
							.subscriptions[eventSource] as any[])?.filter(
	...
	*/
	public async doAction<T = Record<string, any>>(
		...
		if (options?.customEventResponse)
			return this.doActionWithCustomEventResponse(action, args);
	...
	*/
	private async doActionWithCustomEventResponse<T = Record<string, any>>(
		...
		const {
			responseId: sbClientResponse,
			promise,
			controller,
		} = withCustomEventResponse<T>({
			timeout,
	...
	*/
	public async executeCodeTrigger(
		...
		if (options?.customEventResponse)
			return this.executeCodeTriggerWithCustomEventResponse(
			triggerName,
			args,
		);
	...
	* @returns Promise resolving to the Custom event data
	*/
	private async executeCodeTriggerWithCustomEventResponse<
	T = Record<string, any>,
	>(
		triggerName: string,
		args: Record<string, any> = {},
		...
		try {
			const response = await this.executeCodeTrigger(
				triggerName,
				triggerArgs,
			);
			const customEventResponseArgs = await promise;
	...
	*/
	...
	): Promise<TwitchGetEmotesResponse | YouTubeGetEmotesResponse> {
		switch (platform) {
			case "twitch":
	...
	*/
	...
	>(name: K, persisted = true) {
		...
		if (response.status === "ok") {
			if (!response.variables[name])
				return { status: "error", error: "Variable not found" };
			return {
				id: response.id,
	...
	*/
	...
	>(
		...
		const platformToRequest: Record<
		StreamerbotPlatform,
		StreamerbotRequestName
		> = {
	...
	*/
	...
	P extends StreamerbotPlatform = StreamerbotPlatform,
	>(platform: P, userId: U, name: K | null = null, persisted = true) {
		const platformToRequest: Record<
		StreamerbotPlatform,
		StreamerbotRequestName
		> = {
	...
	*/
	public async sendMessage(
		...
		if (
			MULTI_BROADCAST_SUPPORTED.includes(platform) &&
			options.broadcastId
		) {
